...
public class Injector
{
    ...
    // Public method.
    public Injector (String jpgFile, String otherFile)
    {
        injecting(jpgFile, otherFile);
    }

    // Prepares everything for the injection.
    private void injecting (String jpgFilePath, String otherFilePath)
    {
        // Initialize variables.
        final File jpgFile = fileManager.getFile(jpgFilePath);
        final File otherFile = fileManager.getFile(otherFilePath);

        // Checks if the both files are not null.
        if ((jpgFile != null) && (otherFile != null))
        {
            // Use these files for the injection.
            new Inject(jpgFile, otherFile);
        }
        else
        {
            // Shows alert.
